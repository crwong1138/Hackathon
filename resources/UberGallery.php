 ...
 */
    ...
    public function __construct() {
        ...
        $this->_now = time();
        $this->urlbase = 'https://s3-us-west-1.amazonaws.com/buckethead9/';
     ...
     * @access public
     */
    public function createGallery($directory, $relText = 'colorboxXXXX') {
        ...
        $galleryArray['relText'] = $relText . "_______GLORP_____";
    ...
    public function readImageDirectory($directory) {
        ...
        if (!$galleryArray) {
            ...
            if ($__debug) { print "preloop\n"; }
            // Loop through array and add additional info
            foreach ($dirArray as $key => $image) {
              if ($__debug) { print "image: " . $image . "\n"; }
              // Get files relative path
              $AWSpath = $this->urlbase . $image;
              if ($__debug) { print $AWSpath . "\n"; }
              $galleryArray['images'][htmlentities(pathinfo($image, PATHINFO_BASENAME))] =
                array(
                  'file_title'   => str_replace('_', ' ', pathinfo($image, PATHINFO_FILENAME)),
                  'file_path'    => htmlentities($AWSpath),
                  'thumb_path'   => $this->_createThumbnail($this->urlbase, $image)
                );
              if ($__debug) { print "...\n"; }
            }

            if ($__debug) { print "alpha\n"; }
    ...
    private function _readDirectory($directory, $paginate = true) {
      ...
      $result = $s3Client->listObjects($bucket);
      $files = array();
      foreach ($result['Contents'] as $key => $value) {
        if ($__debug) { print $value['Key'] . "\n"; }
        array_push($files, $value['Key']);
      }
      rsort($files);
     ...
     * @access private
     */
    private function _createThumbnail($url, $source, $thumbWidth = NULL, $thumbHeight = NULL, $quality = NULL) {
        $theImage = "$url" . $source;
     ...
     * @access private
     */
    private function _isImage($subject) {
      $pattern = "/jpeg|jpg|tiff|tff|svg|png/i";
...
//        }
    ...
    private function _isFileCached($filePath) {
        return (
           ...
           (($this->_now - filemtime($filePath)) / 60 <= $this->_config['cache_expire']
            || $this->_config['cache_expire'] < 0 )
        );
    }
